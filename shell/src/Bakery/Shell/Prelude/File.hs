...
readF inputPath = do
  ...
  fromPipe $ do
    path <- toPath inputPath
    capped (Pipes.Safe.withFile path ReadMode Pipes.ByteString.fromHandle)

-- | An operation that writes its input to the given file.
writeF :: (MonadMask m, MonadIO m, Path a) => a -> Shell m (Chunk ByteString) Void ()
writeF outputPath = fromPipe $ do
  path <- toPath outputPath
  Pipes.Safe.withFile path WriteMode \handle ->
    consume
